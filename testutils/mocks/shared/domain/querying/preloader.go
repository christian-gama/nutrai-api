...
func (_m *Preloader) Add(field string) querying.Preloader {
	ret := _m.Called(field)

	var r0 querying.Preloader
	if rf, ok := ret.Get(0).(func(string) querying.Preloader); ok {
		r0 = rf(field)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(querying.Preloader)
		}
	}
