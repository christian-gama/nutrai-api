...
func (p *patientImpl) All(ctx context.Context, input repo.AllPatientsInput, preload ...string) (*querying.PaginationOutput[*patient.Patient], error) {
	return p.manager.All(ctx, manager.AllInput[patient.Patient]{Filterer: input.Filterer, Paginator: input.Paginator, Sorter: input.Sorter}, preload...)
}
...
func (p *patientImpl) Find(ctx context.Context, input repo.FindPatientInput, preload ...string) (*patient.Patient, error) {
	return p.manager.Find(ctx, manager.FindInput[patient.Patient]{ID: input.ID}, preload...)
}
